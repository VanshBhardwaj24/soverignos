...
const getRarityStyles = (rarity: Rarity) => {
  switch (rarity) {
    case 'common': return 'border-white/10 text-white/40 bg-white/5';
    case 'uncommon': return 'border-blue-500/30 text-blue-400 bg-blue-500/5 shadow-[0_0_15px_rgba(59,130,246,0.1)]';
    case 'rare': return 'border-purple-500/40 text-purple-400 bg-purple-500/5 shadow-[0_0_20px_rgba(168,85,247,0.15)] scale-[1.02]';
    case 'epic': return 'border-amber-500/50 text-amber-400 bg-amber-500/5 shadow-[0_0_25px_rgba(245,158,11,0.2)] scale-[1.04]';
    case 'legendary': return 'border-red-500/60 text-red-400 bg-red-500/5 shadow-[0_0_35px_rgba(239,68,68,0.3)] scale-[1.06] animate-pulse';
...
export default function Marketplace() {
  const { 
    gold, inventory, wishlist, activeLoadout, customRewards,
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-[1300px] mx-auto pb-24 px-4 font-sans">
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12 bg-white/[0.01] border border-white/5 p-8 rounded-[40px] backdrop-blur-3xl shadow-2xl">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="font-mono text-[10px] tracking-[0.4em] text-white/30 uppercase font-black mb-1">SOVEREIGN EXCHANGE // V2.0</h1>
        ...
        <div className="flex items-center gap-4">
          <div className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl flex flex-col items-end">
            <span className="font-mono text-[9px] text-white/20 uppercase tracking-widest mb-1">Current Balance</span>
            <div className="flex items-center gap-3">
              <Coins size={20} className="text-[var(--stat-wealth)] animate-pulse" />
              <span className="text-3xl font-black text-white tracking-tighter">{gold} <span className="text-xs font-mono text-white/20">GC</span></span>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Main Tabs */}
      <nav className="flex items-center justify-center gap-2 mb-12 bg-white/5 p-1.5 rounded-2xl border border-white/5 w-fit mx-auto backdrop-blur-xl">
        {(['market', 'inventory', 'rewards', 'history'] as Tab[]).map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={cn(
              "px-8 py-3 rounded-xl font-mono text-[11px] font-black tracking-[0.2em] uppercase transition-all",
              activeTab === tab ? "bg-white text-black shadow-xl scale-105" : "text-white/40 hover:text-white hover:bg-white/5"
            )}
          >
            {tab}
          </button>
      ...
      {activeTab === 'rewards' && (
        ...
        >
          <button 
            ...
            className="flex items-center gap-3 px-10 py-5 bg-white text-black rounded-[24px] font-mono text-xs font-black tracking-[0.2em] uppercase hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-white/10"
          >
                 ...
                 <div key={timer.itemId} className="flex-1 min-w-[280px] p-6 bg-gradient-to-br from-white/10 to-white/[0.02] border border-white/10 rounded-3xl backdrop-blur-2xl shadow-xl overflow-hidden relative group">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="font-mono text-[10px] text-white/40 uppercase mt-1">ROI: +{timer.currentROI} XP gained</p>
                      </div>
                      <div className="font-mono text-xs font-black text-white px-2 py-1 bg-white/10 rounded-lg">
                        {timer.timeLeft}
                      </div>
                    </div>
      ...
      {activeTab === 'market' || activeTab === 'rewards' ? (
          ...
          <div className="flex flex-col md:flex-row gap-6 mb-12 items-center">
            <div className="relative flex-1 group w-full">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-6 font-mono text-xs tracking-widest text-white focus:outline-none focus:border-white/30 transition-all placeholder:text-white/10"
              />
            </div>
            ...
            <div className="flex items-center gap-3 overflow-x-auto pb-2 w-full md:w-auto no-scrollbar">
              <button 
                ...
                className={cn(
                  "px-6 py-4 rounded-2xl border font-mono text-[10px] tracking-widest whitespace-nowrap transition-all flex items-center gap-3",
              ...
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 font-mono text-[10px] tracking-widest text-white/60 focus:outline-none focus:border-white/30 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredItems.map((item, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  className={cn(
                    "group relative p-8 rounded-[40px] border transition-all duration-500 flex flex-col justify-between min-h-[480px]",
                ...
                >
                  ...
                  <div>
                    <div className="flex items-start justify-between mb-8">
                      <div className="h-16 w-16 bg-white/10 rounded-3xl flex items-center justify-center text-white border border-white/10 group-hover:scale-110 transition-transform">
                      ...
                      <button 
                        ...
                        className={cn(
                          ...
                          isWishlisted ? "bg-red-500 text-white shadow-lg" : "bg-white/5 text-white/20 hover:bg-white/10"
                        )}
                      >
                    ...
                    <div className="mb-2 flex items-center gap-2">
                       <span className="font-mono text-[8px] font-black tracking-[0.4em] uppercase opacity-40">{item.rarity}</span>
                       <span className="h-1 w-1 rounded-full bg-white/20" />
                       <span className="font-mono text-[8px] font-black tracking-[0.4em] uppercase opacity-40">{item.type}</span>
                    </div>
                    
                    <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter mb-4 group-hover:translate-x-2 transition-transform">
                      {item.name}
                    </h3>
                  ...
                  <div className="space-y-6 pt-6 border-t border-white/5">
                    ...
                    {!canAfford && !owned && (
                      <div className="space-y-2">
                        <div className="flex justify-between font-mono text-[8px] tracking-widest text-white/30 uppercase font-black">
                        ...
                        <p className="font-mono text-[8px] text-white/20 uppercase italic mt-1">Est. 12 days to target</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-[10px] font-mono text-white/20">GC</span>
                      </div>
                      ...
                      <button 
                         ...
                         className={cn(
                           "flex-1 py-4 rounded-2xl font-black font-mono text-[10px] tracking-widest uppercase transition-all flex items-center justify-center gap-2",
                           owned ? "bg-white/5 text-white/20 border border-white/5 cursor-default" :
                           canAfford ? "bg-white text-black hover:scale-105 active:scale-95 shadow-xl" :
            ...
            {filteredItems.length === 0 && (
              <div className="col-span-full py-40 text-center bg-white/[0.01] border border-dashed border-white/5 rounded-[60px]">
                ...
                <h3 className="font-mono text-xl text-white/20 uppercase tracking-widest">No matching protocols found</h3>
                <p className="text-white/10 text-xs mt-2 uppercase tracking-widest font-mono">adjust your filters to expand the search.</p>
              </div>
            )}
          </div>
        </>
        ...
        <div className="space-y-12">
           ...
           <section>
              <h3 className="font-mono text-xs font-black tracking-[0.4em] text-white/30 uppercase mb-8 flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {inventory.filter(id => SHOP_ITEMS.find(i => i.id === id)?.type === 'consumable').map(itemId => {
                  ...
                  return (
                    <div key={Math.random()} className="p-8 bg-white/[0.02] border border-white/5 rounded-[40px] hover:border-white/20 transition-all flex flex-col justify-between h-[240px]">
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <div className="font-mono text-[10px] text-white/20 uppercase">{item.duration}h Duration</div>
                        <button 
                          disabled={isOnCooldown}
                          onClick={() => deployItem(itemId)}
                          className={cn(
                            "px-6 py-3 rounded-xl font-mono text-[9px] font-black tracking-widest uppercase transition-all",
                ...
                {inventory.filter(id => SHOP_ITEMS.find(i => i.id === id)?.type === 'consumable').length === 0 && (
                  <div className="col-span-full py-20 text-center border border-dashed border-white/5 rounded-[40px]">
                    <p className="font-mono text-[10px] text-white/20 uppercase tracking-[0.4em]">No Consumable Assets Available</p>
                  </div>
                )}
              </div>
           </section>
                    ...
                    <div key={itemId} className="p-6 bg-white/[0.01] border border-white/5 rounded-3xl flex items-center justify-between group hover:bg-white/[0.02] transition-all">
                       <div className="flex items-center gap-6">
                         ...
                         <div>
                            ...
                            <p className="text-[10px] text-white/30 mt-1 uppercase font-mono tracking-widest">{item.stat} MODULE // PERMANENT EFFECT</p>
                         </div>
                       </div>
                       <div className="px-4 py-2 rounded-lg bg-[var(--stat-brand)]/10 border border-[var(--stat-brand)]/20 text-[var(--stat-brand)] font-mono text-[10px] font-black uppercase tracking-widest">
                          OPERATIONAL
                       </div>
                    </div>
                  );
        ...
        <div className="py-40 text-center bg-white/[0.01] border border-dashed border-white/5 rounded-[60px]">
           ...
           <h3 className="font-mono text-xl text-white/20 uppercase tracking-widest">Archive Content Restricted</h3>
           <p className="text-white/10 text-xs mt-2 uppercase tracking-widest font-mono">deployment history and audit logs are clearing...</p>
        </div>
      )}

      {/* Footer Disclaimer */}
      <footer className="mt-24 p-8 rounded-[40px] bg-white/[0.01] border border-white/5 text-center">
        <p className="font-mono text-[9px] text-white/10 uppercase tracking-[0.6em] max-w-2xl mx-auto leading-relaxed">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-6">
            <div>
              <label className="font-mono text-[10px] uppercase text-white/20 mb-3 block tracking-widest font-black">Designation</label>
              <input 
                value={formData.name}
            ...
            <div>
              <label className="font-mono text-[10px] uppercase text-white/20 mb-3 block tracking-widest font-black">Description & Requirements</label>
              <textarea 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="font-mono text-[10px] uppercase text-white/20 mb-3 block tracking-widest font-black">Cost (GC)</label>
                <input 
                  type="number"
                  value={formData.cost}
                  onChange={e => setFormData({ ...formData, cost: parseInt(e.target.value) || 0 })}
                  className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white font-mono text-sm focus:border-white/30 outline-none transition-all"
                />
              </div>
              <div>
                <label className="font-mono text-[10px] uppercase text-white/20 mb-3 block tracking-widest font-black">Rarity</label>
                <select 
                  value={formData.rarity}
                  onChange={e => setFormData({ ...formData, rarity: e.target.value as any })}
                  className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white font-mono text-xs focus:border-white/30 outline-none transition-all appearance-none"
                >
            ...
            <div className="pt-6 border-t border-white/5 flex gap-4">
              <button 
                onClick={onClose}
                className="flex-1 py-4 font-mono text-[10px] font-black tracking-widest text-white/40 uppercase hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button 
                ...
                className="flex-[2] py-4 bg-white text-black rounded-2xl font-mono text-[10px] font-black tracking-widest uppercase hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-30 shadow-xl shadow-white/5"
              >
                Confirm Protocol
              </button>
            </div>
