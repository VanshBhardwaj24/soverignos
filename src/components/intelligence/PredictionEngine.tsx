    ...
    <section className="p-8 rounded-3xl border border-[var(--border-default)] bg-white/[0.02] backdrop-blur-xl h-full">
      <div className="flex justify-between items-start mb-10">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-mono text-xs tracking-[0.2em] uppercase text-[var(--text-secondary)]">Prediction Engine</h2>
          </div>
          <p className="font-mono text-[9px] text-white/20 uppercase tracking-[0.2em]">Next 72-Hour Horizon</p>
        </div>
        ...
        {predictions.map((pred: any, i: number) => (
          <div key={i} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 group hover:border-orange-500/30 transition-all">
            <div className="flex justify-between items-center mb-3">
              <span className="font-mono text-[11px] text-white/80">{pred.text}</span>
              <span className={cn(
                "px-2 py-0.5 rounded-full font-mono text-[8px] font-black uppercase",
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="font-mono text-[10px] font-bold text-white min-w-[30px]">{pred.prob}%</span>
            </div>
            ...
            {pred.beatable && (
              <div className="mt-4 flex items-center justify-between pt-3 border-t border-white/5 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="font-mono text-[9px] text-white/40 uppercase">Recommended Antidote</span>
                <button className="flex items-center gap-1 font-mono text-[9px] text-orange-400 uppercase font-black">
      ...
      <div className="mt-8 pt-8 border-t border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[9px] text-white/40 uppercase tracking-widest">falsifiable layer active</span>
        </div>
        <span className="font-mono text-[9px] text-white/20 uppercase">Last updated: Just Now</span>
      </div>
    </section>
  );
};
