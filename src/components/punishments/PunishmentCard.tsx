      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex flex-col items-end gap-1">
            <span className="font-mono text-[9px] text-white/20 uppercase font-black tracking-widest bg-white/5 px-3 py-1 rounded-full border border-white/5">
        ...
        <div className="mb-6 flex-1">
          <h4 className="font-mono text-sm font-black text-white uppercase mb-2 tracking-tight flex items-center gap-2">
            {punishment.title}
          </h4>
          ...
          {punishment.penalty && (
            <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-red-500/10 border border-red-500/20">
               <span className="font-mono text-[10px] text-red-500 font-black">SANCTION: -{punishment.penalty} GC</span>
            </div>
          )}
        </div>
        ...
        {isAwaiting ? (
          <div className="space-y-3">
            <p className="text-[9px] font-mono font-black text-white/40 uppercase tracking-widest mb-1 italic">Select Resolution Protocol:</p>
            {punishment.options?.map((option: any) => (
              ...
              >
                <div className="flex items-center justify-between gap-4 mb-1">
                  <span className="font-mono text-[10px] font-black text-white uppercase group-hover/opt:text-[var(--accent)] transition-colors">{option.title}</span>
                  <span className="text-[9px] font-mono text-white/20 uppercase">+{option.xpReward} XP</span>
                </div>
          ...
          <button 
            ...
            disabled={!!(punishment.type === 'consequential' && punishment.status === 'active' && punishment.questId)}
            className={cn(
               "w-full py-4 bg-white text-black rounded-2xl font-mono text-[10px] font-black tracking-[0.2em] uppercase hover:brightness-90 active:scale-[0.98] transition-all",
               (punishment.type === 'consequential' && punishment.status === 'active' && punishment.questId) && "opacity-50 cursor-not-allowed bg-white/20 text-white/40 border border-white/10"
            )}
          >
            {(punishment.type === 'consequential' && punishment.status === 'active' && punishment.questId) ? "Resolving via Protocol..." : "Certify Resolution"}
          </button>
        ) : (
