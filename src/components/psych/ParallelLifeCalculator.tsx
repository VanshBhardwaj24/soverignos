...
export function ParallelLifeCalculator({ compact = false }: ParallelLifeCalculatorProps) {
    ...
    return (
      <div className="p-6 bg-white/[0.02] border border-white/[0.06] rounded-[28px]">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-mono text-[9px] tracking-[0.25em] text-white/30 uppercase font-black">Parallel Life</h3>
            <p className="font-mono text-sm font-light text-white">Current vs. Consistent You</p>
          </div>
          <div className="text-right">
            <p className="font-mono text-[8px] text-white/20 uppercase">Freedom delayed by</p>
            <p className="font-mono text-lg font-black text-red-400">{freedomDelayDays}d</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="font-mono text-[10px] tracking-[0.3em] text-white/30 uppercase font-black mb-1">Reality Divergence Engine</h2>
        <p className="font-mono text-2xl font-light text-white">PARALLEL LIFE CALCULATOR</p>
      ...
      >
        <div>
          <p className="font-mono text-[9px] uppercase tracking-widest text-white/40">Estimated Freedom Delay</p>
          <p className={`font-mono text-4xl font-black mt-1 ${freedomDelayDays === 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="p-6 bg-white/[0.02] border border-white/[0.06] rounded-[28px] space-y-1">
        ...
        <div className="grid grid-cols-5 gap-4 px-2 pb-4 border-b border-white/5">
          <span className="col-span-2 font-mono text-[8px] text-white/20 uppercase tracking-widest">Metric</span>
          <span className="font-mono text-[8px] text-white/20 uppercase tracking-widest text-center">Current You</span>
          <span className="font-mono text-[8px] text-green-400/60 uppercase tracking-widest text-center">Consistent You</span>
          <span className="font-mono text-[8px] text-white/20 uppercase tracking-widest text-right">Gap</span>
        </div>
          ...
          return (
            ...
            >
              <div className="col-span-2 flex items-center gap-3">
                ...
                <span className="font-mono text-[10px] text-white/60">{m.label}</span>
              </div>
              <div className="text-center">
                <span className={`font-mono text-sm font-bold ${gap > 0 ? 'text-red-400' : 'text-white'}`}>
                  {m.actual}
                </span>
              ...
              <div className="text-center">
                <span className="font-mono text-sm font-bold text-green-400">{m.target}</span>
              </div>
              <div className="text-right flex items-center justify-end gap-1">
                {gap > 0 ? (
                  <>
                    <span className="font-mono text-sm font-black text-red-400">-{gap}</span>
                  </>
                ) : (
      ...
      <p className="font-mono text-[8px] text-white/10 text-center uppercase tracking-widest">
