  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h3 className="font-bold text-[10px] tracking-[0.3em] text-white/30 uppercase font-black mb-1">Freedom Audit</h3>
          <p className="font-bold text-2xl font-light text-white">DEPENDENCY MIRROR</p>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl font-bold text-[9px] text-white/40 uppercase hover:text-white hover:border-white/20 transition-all"
        >
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={s.label} className={`p-4 bg-white/[0.02] border ${s.border} rounded-2xl text-center`}>
              <p className={`font-bold text-2xl font-black ${s.color}`}>{s.count}</p>
              <p className="font-bold text-[8px] text-white/20 uppercase tracking-widest mt-1">{s.label}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="space-y-3">
        {dependencyItems.length === 0 ? (
          <div className="py-14 text-center border border-dashed border-white/10 rounded-[24px]">
            ...
            <p className="font-bold text-[9px] text-white/20 uppercase tracking-[0.3em]">No dependencies logged</p>
            <p className="font-bold text-[8px] text-white/10 mt-1">What are you still dependent on?</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="font-bold text-[10px] font-black text-white">{item.area}</span>
                    <span className={`font-bold text-[7px] uppercase tracking-widest ${s.color} opacity-70`}>{s.label}</span>
                  </div>
                  <div className="flex items-center gap-1 font-bold text-[9px] text-white/30">
                    <ArrowRight size={10} />
                  ...
                  {(['unchanged', 'improved', 'resolved'] as DependencyItem['status'][]).map(status => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'px-3 py-1 rounded-lg border font-bold text-[7px] uppercase tracking-wider transition-all',
                          item.status === status
      ...
      {dependencyItems.length === 0 && (
        <div className="space-y-2">
          <p className="font-bold text-[8px] text-white/10 uppercase tracking-widest text-center">Suggested dependencies to track:</p>
          {['Family for accommodation', 'No income stream', 'Zero public presence', 'No savings buffer'].map(s => (
            <button
              ...
              className="w-full text-left px-4 py-3 bg-white/[0.01] border border-white/[0.04] rounded-xl font-bold text-[9px] text-white/20 hover:text-white/40 hover:border-white/10 transition-all flex items-center justify-between group"
            >
              {s}
        ...
        <div className="fixed inset-0 z-[800] bg-black/80 backdrop-blur-md flex items-center justify-center p-6">
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-md bg-[#0a0a0a] border border-white/10 rounded-[32px] p-8 space-y-5">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-sm font-black text-white uppercase">Add Dependency</h4>
              <button onClick={() => setShowAdd(false)} className="text-white/20 hover:text-white"><X size={18} /></button>
            </div>
            <div>
              <label className="block font-bold text-[8px] text-white/20 uppercase tracking-widest mb-2">Dependency Area</label>
              <input value={newArea} onChange={e => setNewArea(e.target.value)} placeholder="e.g. Family for accommodation" className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white text-sm font-bold outline-none focus:border-[var(--stat-mind)]" />
            </div>
            <div>
              <label className="block font-bold text-[8px] text-white/20 uppercase tracking-widest mb-2">Freedom Target</label>
              <input value={newTarget} onChange={e => setNewTarget(e.target.value)} placeholder="e.g. When I get first paycheck" className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white text-sm font-bold outline-none focus:border-[var(--stat-mind)]" />
            </div>
            <div className="flex gap-3">
              <button onClick={handleAdd} disabled={!newArea || !newTarget} className="flex-1 py-3 bg-white text-black font-bold font-black text-[9px] uppercase rounded-xl hover:opacity-90 disabled:opacity-30 transition-all">Add</button>
              <button onClick={() => setShowAdd(false)} className="px-6 py-3 bg-white/5 text-white/40 font-bold text-[9px] uppercase rounded-xl hover:text-white transition-all">Cancel</button>
            </div>
          </motion.div>
        </div>
      )}
