  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          <h2 className="font-mono text-2xl font-black text-white uppercase italic">Temporal Letters</h2>
          <p className="font-mono text-[10px] text-white/40 uppercase tracking-[0.2em] mt-2">Write to the version of you that won.</p>
        </div>
        <button
          onClick={() => setIsWriting(true)}
          className="px-6 py-3 bg-white text-black font-mono text-[10px] uppercase font-black tracking-widest rounded-2xl hover:brightness-90 transition-all flex items-center justify-center gap-2"
        >
            ...
            <div className="bg-[#111] border border-white/10 rounded-[32px] p-8 mb-12 shadow-2xl relative">
              ...
              <div className="mb-6 flex gap-4">
                <button
                  ...
                  className={cn(
                    "px-4 py-3 rounded-xl font-mono text-[10px] uppercase tracking-widest transition-all flex items-center gap-2",
              ...
              <textarea
                ...
                className="w-full h-64 bg-white/5 border border-white/10 rounded-2xl p-6 text-white font-mono text-sm leading-relaxed outline-none focus:border-white/20 resize-none mb-6"
              />

              <button
                onClick={handleSealLetter}
                disabled={!content.trim()}
                className="w-full py-4 bg-white text-black font-mono font-black text-[10px] tracking-[0.2em] uppercase rounded-xl hover:bg-white/90 disabled:opacity-50 flex items-center justify-center gap-2 transition-all"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* SEALED LETTERS */}
        <div>
          <h3 className="font-mono text-[10px] text-amber-500/80 uppercase tracking-[0.3em] font-black mb-6 flex items-center gap-2">
          ...
          <div className="space-y-4">
            {sealedLetters.length === 0 ? (
              <div className="p-8 border border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center opacity-30 text-center">
                ...
                <p className="font-mono text-[9px] uppercase tracking-widest">No sealed letters.</p>
              </div>
            ) : (
              sealedLetters.map(letter => (
                <div key={letter.id} className="p-6 bg-amber-500/5 border border-amber-500/20 rounded-3xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex items-start justify-between">
                    <div>
                      <p className="font-mono text-[9px] text-amber-500/60 uppercase tracking-widest mb-1">
                        Sealed on {new Date(letter.writtenAt).toLocaleDateString()}
                      </p>
                      <h4 className="font-mono text-sm text-amber-400 uppercase font-bold">
        ...
        {/* DELIVERED LETTERS */}
        <div>
          <h3 className="font-mono text-[10px] text-[var(--stat-mind)]/80 uppercase tracking-[0.3em] font-black mb-6 flex items-center gap-2">
          ...
          <div className="space-y-4">
            {openLetters.length === 0 ? (
              <div className="p-8 border border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center opacity-30 text-center">
                ...
                <p className="font-mono text-[9px] uppercase tracking-widest">Timeline intact. No messages received yet.</p>
              </div>
            ) : (
              openLetters.map(letter => (
                <div key={letter.id} className="p-8 bg-white/[0.02] border border-white/10 rounded-3xl group">
                  <div className="flex items-center justify-between mb-6">
                    <span className="font-mono text-[9px] text-white/40 uppercase tracking-widest px-3 py-1 bg-white/5 rounded-full">
                      Written: {new Date(letter.writtenAt).toLocaleDateString()}
                    </span>
                    <span className="font-mono text-[9px] text-[var(--stat-mind)] uppercase tracking-widest flex items-center gap-1">
                      <Check size={12} /> Delivered
                    </span>
                  </div>
                  <div className="prose prose-invert prose-p:font-mono prose-p:text-sm prose-p:text-white/70 prose-p:leading-relaxed max-w-none">
