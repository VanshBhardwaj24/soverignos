    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-bold text-[10px] tracking-[0.3em] text-white/30 uppercase font-black">Accountability Protocol</h3>
            <p className="font-bold text-xl font-light text-white">DEAD MAN'S SWITCH</p>
          </div>
        </div>
      ...
      <p className="font-bold text-[10px] text-white/25 leading-relaxed">
      ...
      {deadManConfig.enabled && (
        <div className={cn(
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className={cn(
                'font-bold text-[10px] font-black uppercase tracking-widest',
              ...
              <p className="font-bold text-[8px] text-white/20 mt-0.5">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-white text-black rounded-xl font-bold text-[9px] font-black tracking-widest uppercase hover:opacity-90 transition-all flex items-center gap-2"
          >
      ...
      <div className="p-6 bg-white/[0.02] border border-white/[0.06] rounded-[24px] space-y-4">
        <div className="flex items-center justify-between mb-4">
          <p className="font-bold text-[9px] text-white/30 uppercase tracking-widest">Contact Configuration</p>
          <button onClick={() => setEditing(!editing)} className="font-bold text-[9px] text-white/30 hover:text-white transition-colors uppercase">
            {editing ? 'Cancel' : 'Edit'}
          </button>
        </div>
        ...
        <AnimatePresence mode="wait">
          {editing ? (
            <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              <div>
                <label className="block font-bold text-[8px] text-white/20 uppercase tracking-widest mb-2">Contact Name</label>
                <div className="relative">
                  <User size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20" />
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Mom, Best Friend" className="w-full pl-9 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl font-bold text-sm text-white outline-none focus:border-[var(--stat-mind)]" />
                </div>
              </div>
              <div>
                <label className="block font-bold text-[8px] text-white/20 uppercase tracking-widest mb-2">Contact Email</label>
                <div className="relative">
                  <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20" />
                  <input value={email} onChange={e => setEmail(e.target.value)} placeholder="contact@example.com" type="email" className="w-full pl-9 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl font-bold text-sm text-white outline-none focus:border-[var(--stat-mind)]" />
                </div>
              </div>
              <div>
                <label className="block font-bold text-[8px] text-white/20 uppercase tracking-widest mb-2">
                ...
                <div className="relative">
                  ...
                  />
                  <div className="flex justify-between font-bold text-[7px] text-white/20 mt-1"><span>12h</span><span>96h</span></div>
                </div>
              </div>
              <button onClick={handleSave} className="w-full py-3 bg-white text-black font-bold font-black text-[9px] uppercase tracking-widest rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2">
            ...
            <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              {[
                ...
                <div key={row.label} className="flex items-center justify-between">
                  <span className="font-bold text-[9px] text-white/20 uppercase tracking-widest">{row.label}</span>
                  <span className="font-bold text-[10px] text-white/60">{row.value}</span>
                </div>
              ))}
            </motion.div>
          )}
      ...
      <div className="p-4 bg-white/[0.01] border border-white/[0.04] rounded-2xl">
        <p className="font-bold text-[8px] text-white/15 leading-relaxed">
